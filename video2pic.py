...
def save_img():
    # video_path = r'D:\embeded\BadApple'
    # videos = os.listdir(video_path)
    videos = load_video(r'D:\embeded\BadApple\video\BadApple.mp4')
    for video_name in videos:
        ...
        if vc.isOpened():  # 判断是否正常打开
            ...
            rval = False
        timeF = 3  # 视频帧计数间隔频率 原视频60fps
        while rval:  # 循环读取视频帧
            rval, frame = vc.read()
            if (c % timeF == 0):  # 每隔timeF帧进行存储操作
                if frame is not None:
                    cv2.imwrite("D:\embeded\BadApple\pic\p"  + '_' + str(c/timeF) +
                            '.jpg', frame)
            c = c + 1
            cv2.waitKey(1)
        vc.release()
